...
import tkinter.messagebox as messagebox # filedialog allows user to select where they want to save the file.
...
current_textcolor = "yellow"
current_font_family = "Liberation Mono"
current_font_size = 12
fontColor = current_textcolor
current_bgcolor = "#191970"
fontBackground= current_bgcolor
...
def make_tag():
	...
	global current_bgcolor
	#print(current_bgcolor)
	master.configure(background=current_bgcolor)
...
def save_as(event=None):
	...
	if file_name == "":
		save()
		return "break"
	f = filedialog.asksaveasfile(mode='w',defaultextension=".txt",filetypes=file_types)
	if f is None: 
		return
...
def paste(event=None):
	...
	pasted = master.clipboard_get().split('\n')
	#print("IN PASTE")
	for toPaste in pasted:
		#print(toPaste)
		text.insert(INSERT,toPaste)
		text.insert(END,'\n')
...
def select_all(event=None):
	all_lines = text.get("1.0", "end").split('\n')
	...
	for idx in range(1, lines):
		end = len(all_lines[idx-1])+1
		#print(end)
		text.tag_add('sel', f"{idx}.0",f"{idx}.{end}")
...
def change_color():
	...
	color_name = color[1]
	global fontColor
	global current_textcolor
	fontColor = color_name
	current_textcolor = fontColor
	current_tags = text.tag_names()
...
def find_text(event=None):
	...
	x = master.winfo_x()
	y = master.winfo_y()
	search_toplevel.geometry("+%d+%d" %(x+1240,y+90))
	# search_toplevel.overrideredirect(True)
...
def change_font(event):
	...
	text.configure(font=(f,current_font_size))
	make_tag()
...
# TOOLBAR
toolbar = Frame(master, pady=2, bg="#AA2704")

# TOOLBAR BUTTONS
# new
new_button = Button(name="toolbar_b2", borderwidth=1, command=new, width=20, height=20, background="#84F1F1")
photo_new = Image.open("icons/new.png")
photo_new = photo_new.resize((18, 18), Image.Resampling.LANCZOS)
...
save_button = Button(name="toolbar_b1", borderwidth=1,
                     command=save, width=20, height=20, bg="#84F1F1")
photo_save = Image.open("icons/save.png")
photo_save = photo_save.resize((18, 18), Image.Resampling.LANCZOS)
...
open_button = Button(name="toolbar_b3", borderwidth=1,
                     command=open_file, width=20, height=20, bg="#84F1F1")
photo_open = Image.open("icons/open.png")
photo_open = photo_open.resize((18, 18), Image.Resampling.LANCZOS)
...
copy_button = Button(name="toolbar_b4", borderwidth=1,
                     command=copy, width=20, height=20, bg="#84F1F1")
photo_copy = Image.open("icons/copy.png")
photo_copy = photo_copy.resize((18, 18), Image.Resampling.LANCZOS)
image_copy = ImageTk.PhotoImage(photo_copy)
copy_button.config(image=image_copy)
copy_button.pack(in_=toolbar, side="left", padx=(40,4), pady=4)

#cut
cut_button = Button(name="toolbar_b5", borderwidth=1,
                    command=cut, width=20, height=20, bg="#84F1F1")
photo_cut = Image.open("icons/cut.png")
photo_cut = photo_cut.resize((18, 18), Image.Resampling.LANCZOS)
...
paste_button = Button(name="toolbar_b6", borderwidth=1,
                      command=paste, width=20, height=20, bg="#84F1F1")
photo_paste = Image.open("icons/paste.png")
photo_paste = photo_paste.resize((18, 18), Image.Resampling.LANCZOS)
...
redo_button = Button(name="toolbar_b7", borderwidth=1,
                     command=redo, width=20, height=20, bg="#84F1F1")
photo_redo = Image.open("icons/redo.png")
photo_redo = photo_redo.resize((18, 18), Image.Resampling.LANCZOS)
image_redo = ImageTk.PhotoImage(photo_redo)
redo_button.config(image=image_redo)
redo_button.pack(in_=toolbar, side="left", padx=(40,4), pady=4)

# undo
undo_button = Button(name="toolbar_b8", borderwidth=1,
                     command=undo, width=20, height=20, bg="#84F1F1")
photo_undo = Image.open("icons/undo.png")
photo_undo = photo_undo.resize((18, 18), Image.Resampling.LANCZOS)
...
# FORMATBAR BUTTONS
#bold
bold_button = Button(name="formatbar_b1", borderwidth=1, command=bold,
                     width=20, height=20, pady=10, padx=10, bg="#84F1F1")
photo_bold = Image.open("icons/bold.png")
photo_bold = photo_bold.resize((18, 18), Image.Resampling.LANCZOS)
image_bold = ImageTk.PhotoImage(photo_bold)
bold_button.config(image=image_bold)
bold_button.pack(in_=formattingbar, side="left", padx=(40,4), pady=4)

# italic
italic_button = Button(name="formatbar_b2", borderwidth=1,
                       command=italic, width=20, height=20, bg="#84F1F1")
photo_italic = Image.open("icons/italic.png")
photo_italic = photo_italic.resize((18, 18), Image.Resampling.LANCZOS)
...
# underline
underline_button = Button(name="formatbar_b3", borderwidth=1,
                          command=underline, width=20, height=20, bg="#84F1F1")
photo_underline = Image.open("icons/underline.png")
photo_underline = photo_underline.resize((18, 18), Image.Resampling.LANCZOS)
...
strike_button = Button(name="formatbar_b4", borderwidth=1,
                       command=strike, width=20, height=20, bg="#84F1F1")
photo_strike = Image.open("icons/strike.png")
photo_strike = photo_strike.resize((18, 18), Image.Resampling.LANCZOS)
image_strike = ImageTk.PhotoImage(photo_strike)
strike_button.config(image=image_strike)
strike_button.pack(in_=formattingbar, side="left", padx=(4, 4), pady=4)

# font_color
font_color_button = Button(name="formatbar_b5", borderwidth=1,
                           command=change_color, width=20, height=20, bg="#84F1F1")
photo_font_color = Image.open("icons/font-color.png")
photo_font_color = photo_font_color.resize((18, 18), Image.Resampling.LANCZOS)
image_font_color = ImageTk.PhotoImage(photo_font_color)
font_color_button.config(image=image_font_color)
font_color_button.pack(in_=formattingbar, side="left", padx=(40,4), pady=4)

# highlight
highlight_button = Button(name="formatbar_b6", borderwidth=1,
                          command=highlight, width=20, height=20, bg="#84F1F1")
photo_highlight = Image.open("icons/highlight.png")
photo_highlight = photo_highlight.resize((18, 18), Image.Resampling.LANCZOS)
...
all_fonts = StringVar()
font_menu = Combobox(formattingbar, textvariable=all_fonts, state="readonly")
font_menu.pack(in_=formattingbar, side="left", padx=(40,4), pady=4)
font_menu['values'] = sorted(list(tkinter.font.families()))
...
# size combobox
all_size = StringVar()
size_menu = Combobox(formattingbar, textvariable=all_size,
...
size_menu.current(1)
align_left_button = Button(name="formatbar_b9", borderwidth=1,
                           command=align_left, width=20, height=20, bg="#84F1F1")
photo_align_left = Image.open("icons/align-left.png")
photo_align_left = photo_align_left.resize((18, 18), Image.Resampling.LANCZOS)
image_align_left = ImageTk.PhotoImage(photo_align_left)
align_left_button.config(image=image_align_left)
align_left_button.pack(in_=formattingbar, side="left", padx=(40, 4), pady=4)



# align_center
align_center_button = Button(name="formatbar_b7", borderwidth=1,
                             command=align_center, width=20, height=20, bg="#84F1F1")
photo_align_center = Image.open("icons/align-center.png")
photo_align_center = photo_align_center.resize((18, 18), Image.Resampling.LANCZOS)
...
# align_right
align_right_button = Button(name="formatbar_b10", borderwidth=1,
                            command=align_right, width=20, height=20, bg="#84F1F1")
photo_align_right = Image.open("icons/align-right.png")
photo_align_right = photo_align_right.resize((18, 18), Image.Resampling.LANCZOS)
...
# STATUS BAR
align_justify_button = Button(name="formatbar_b8", borderwidth=1,
                              command=align_justify, width=20, height=20, bg="#84F1F1")
photo_align_justify = Image.open("icons/align-justify.png")
photo_align_justify = photo_align_justify.resize((18, 18), Image.Resampling.LANCZOS)
...
find_button = Button(name="toolbar_b9", borderwidth=1,
                     command=find_text, width=20, height=20, bg="#84F1F1")
photo_find = Image.open("icons/find.png")
photo_find = photo_find.resize((18, 18), Image.Resampling.LANCZOS)
image_find = ImageTk.PhotoImage(photo_find)
find_button.config(image=image_find)
find_button.pack(in_=toolbar, side="left", padx=(480,4), pady=4)
...
text = Text(wrap="word", font=("Liberation Mono", 12), background=current_bgcolor, borderwidth=1, highlightthickness=0 , undo= True,foreground=current_textcolor,insertbackground="#e6e6eb")
...
menu = Menu(master,bg="#191970",font=("Merriweather 13 bold"),foreground="white")
master.config(menu=menu)

# File menu.
file_menu = Menu(menu,bg="#191970",font=("Merriweather 13 bold"),foreground="white")
...
# Edit Menu.
edit_menu = Menu(menu,bg="#191970",font=("Merriweather 13 bold"),foreground="white")
...
#Tool Menu
tool_menu = Menu(menu,bg="#191970",font=("Merriweather 13 bold"),foreground="white")
...
help_menu = Menu(menu,background="#191970",font=("Merriweather 13 bold"),foreground="white")
